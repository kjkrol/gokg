...
func TestFloat64VectorMath_Length(t *testing.T) {
	...
	expected := 5.0
	result := FLOAT_64_VecEC_MATH.Length(v)
	if result != expected {
...
func TestIntVectorMath_Length(t *testing.T) {
	v := Vec[int]{X: 3, Y: 4}
	expected := 5
	result := INT_VecEC_MATH.Length(v)
	if result != expected {
		t.Errorf("expected %v, got %v", expected, result)
	}
}
...
func TestFloat64VectorMath_Clamp(t *testing.T) {
	...
	FLOAT_64_VecEC_MATH.Clamp(&v, bounds)
	if v != expected {
...
func TestFloat64VectorMath_Wrap(t *testing.T) {
	...
	for _, c := range cases {
		...
		FLOAT_64_VecEC_MATH.Wrap(&Vec, c.offset)
		if Vec != c.expected {
...
func TestIntVectorMath_Wrap(t *testing.T) {
	...
	for _, c := range cases {
		...
		INT_VecEC_MATH.Wrap(&Vec, c.offset)
		if Vec != c.expected {
			t.Errorf("expected %v, got %v", c.expected, Vec)
		}
	}
