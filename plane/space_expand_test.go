...
func TestEuclidean2DExpand(t *testing.T) {
	...
	aabb := NewAABB(vec(2, 3), 3, 4)
	euclidean.Expand(&aabb, 2)
	expectAABBState(t, aabb, vec(0, 1), vec(7, 9), map[FragPosition][2]geom.Vec{})
}

func TestEuclidean2DExpandCornerCase(t *testing.T) {
	euclidean := NewEuclidean2D(10, 10)
	aabb := NewAABB(vec(0, 0), 2, 2)
	euclidean.Expand(&aabb, 2)
	expectAABBState(t, aabb, vec(0, 0), vec(4, 4), map[FragPosition][2]geom.Vec{})
}
...
func TestToroidal2DExpandCornerCase(t *testing.T) {
	...
	expectAABBState(t, aabb, vec(8, 8), vec(10, 10), convertFragments(map[FragPosition][2]geom.Vec{
...
func TestToroidal2DExpandThenIntersects(t *testing.T) {
	...
	aabb1 := NewAABB(vec(5, 5), 10, 10)
	aabb2 := NewAABB(vec(96, 96), 10, 10)
