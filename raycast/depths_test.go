...
func TestDepths_TwoSamplesAreTheConeEdges(t *testing.T) {
	...
	s.put(near, 1200, 1200, 30, 30) // off the axis, reachable only near one edge
...
// Depths borrows the scan's candidates and its active-set buffer, so it must
// not disturb what Entities and Outline read.
func TestDepths_DoesNotDisturbTheExactSweep(t *testing.T) {
	...
	v := &raycast.View{}
	v.Scan(s, eye, cone)
	v.Depths(24, nil) // in between, on purpose
...
// The exact sweep drops critical angles that crowd together, but Depths must
// hand back exactly k samples however fine the step — the caller sized a buffer
// for them.
func TestDepths_ReturnsExactlyKEvenBelowTheSweepsTolerance(t *testing.T) {
	...
	v := &raycast.View{}
	if !v.Scan(s, eye, eastward(math.Pi/4, 10)) { // short range, so a coarse eps
		t.Fatal("Scan failed")
	}
	const k = 200
